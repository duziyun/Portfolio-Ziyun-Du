...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        """computes the most likely position of each ghost"""
        ghost_position_dic = {}
        for ghost_distribution in livingGhostPositionDistributions:
            largest_prob = float("-inf")
            largest_position = None
            # print "largest_prob ",largest_prob
            # print ghost_distribution
            for position, prob in ghost_distribution.items():
                if prob > largest_prob:
                    largest_prob = prob
                    largest_position = position
                    # print "prob == largest_prob",prob
            ghost_position_dic[largest_position] = largest_prob
        # print "ghost_position_dic",ghost_position_dic

        """computes most likely distance of each ghost"""
        ghost_distance_dic = {}
        for position in ghost_position_dic:
            ghost_distance_dic[position] = self.distancer.getDistance(position, pacmanPosition)

        # print "ghost_distance_dic",ghost_distance_dic
        # print "distancelist ",distancelist
        closest_distance = float("inf")
        closest_position = None
        for positionn, distance in ghost_distance_dic.items():
            if distance < closest_distance:
                closest_position = positionn
                closest_distance = distance
        """chooses an action that brings Pacman closer to the closest ghost"""
        bestaction=None
        bestdistance=float("inf")
        for actionn in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, actionn)
            action_distance=self.distancer.getDistance(successorPosition, closest_position)
            if action_distance<bestdistance:
                bestaction=actionn
                bestdistance=action_distance
        return bestaction
